...
def main(args):
    ...
    def augment_take_mask(data):
        return (data['image'], tf.one_hot(tf.cast(data['mask'], dtype=tf.uint8), 2, axis=2))
    ...
    train = train.map(
        augment_take_mask).take(-1).map(augment_bigger, num_parallel_calls=10).cache()
    train = train.shuffle(l).map(augment_flip).map(augment_crop).batch(
        args.batch_size).prefetch(buffer_size=tf.data.AUTOTUNE)
    ...
    x = keras.layers.Convolution2D(
        2, 3, padding='same')(efficientnet_b0.output[1])
