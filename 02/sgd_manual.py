...
class Model(tf.Module):
    ...
    def train_epoch(self, dataset):
        ...
        for batch in tqdm(dataset.batches(self._args.batch_size)):
            ...
            # má mít rozměr 64(batch), 10(output)
            b2_derivative = outputs - desired_outputs

            # má mít rozměr 64(batch), 20(hidden), 10(output)
            W2_derivative = h_1[:, :, tf.newaxis] * \
                b2_derivative[:, tf.newaxis, :]

            # má mít rozměr 64 , 20
            h_1_derivative = b2_derivative  @ tf.transpose(self._W2)

            tanh_derivative = 1 - tf.pow(tf.tanh(mezivysledek), 2)  # 64, 20

            I1_derivative_local = h_1_derivative * tanh_derivative  # 64, 20

            W1_derivative = inputs[:, :, tf.newaxis] * \
                I1_derivative_local[:, tf.newaxis, :]  # 64, 746 , 20
            ...
            self._W2.assign_sub(self._args.learning_rate *
                                tf.math.reduce_mean(W2_derivative, axis=0))
            ...
            self._b2.assign_sub(self._args.learning_rate *
                                tf.math.reduce_mean(b2_derivative, axis=0))

            self._W1.assign_sub(self._args.learning_rate *
                                tf.math.reduce_mean(W1_derivative, axis=0))
            self._b1.assign_sub(self._args.learning_rate *
                                tf.math.reduce_mean(I1_derivative_local, axis=0))
