...
class Model(tf.Module):
    ...
    def predict(self, inputs):
        ...
        y_1 = tf.nn.tanh(inputs @ self._W1 + self._b1)
        result = tf.nn.softmax(y_1 @ self._W2 + self._b2)
        ...
        for batch in dataset.batches(self._args.batch_size):
            ...
            with tf.GradientTape() as tape:
                ...
                loss = tf.keras.losses.categorical_crossentropy(tf.keras.utils.to_categorical(
                    batch["labels"], num_classes=probabilities.shape[1]), probabilities) / batch["labels"].shape[0]
            ...
            for variable, gradient in zip(self.trainable_variables, gradients):
