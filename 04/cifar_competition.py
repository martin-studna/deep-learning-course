...
def main(args):
    ...
    train = train.shuffle(len(cifar.train.data["images"]), seed=args.seed).map(train_augment, num_parallel_calls=200, deterministic=False).prefetch(len(cifar.train.data["images"])//2).batch(args.batch_size)
        
    model.fit(train, verbose=1, callbacks=callback,  validation_data=(
        cifar.dev.data["images"], y_dev), epochs=args.epochs, workers=400, use_multiprocessing=True, max_queue_size=len(cifar.train.data["images"]))
