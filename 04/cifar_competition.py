...
def main(args):
    ...
    train = train.shuffle(len(cifar.train.data["images"]), seed=args.seed).map(train_augment, num_parallel_calls=4, deterministic=False).prefetch(len(cifar.train.data["images"])//2).batch(args.batch_size)
