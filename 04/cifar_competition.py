...
def main(args):
    ...
    predict = True  # jestli chceme trénovat nebo predikovat
    if not predict:
        # TODO: Create the model and train it
        v = 0.5
        #model = MyModel()
        input = Input(shape=(32, 32, 3))
        x = Conv2D(32//v, (3, 3), activation='relu', kernel_initializer='he_uniform',
                   padding='same', kernel_regularizer=l2(args.l2))(input)
        x = BatchNormalization()(x)
        x = Conv2D(64//v, (3, 3), activation='relu', kernel_regularizer=l2(args.l2),
        ...
        x = MaxPooling2D((2, 2))(x)
        x = Dropout(0.3)(x)
        x = Conv2D(128//v, (3, 3), activation='relu', kernel_regularizer=l2(args.l2),
                   kernel_initializer='he_uniform', padding='same')(x)
        x = BatchNormalization()(x)
        x = Conv2D(128//v, (3, 3), activation='relu', kernel_regularizer=l2(args.l2),
                   kernel_initializer='he_uniform', padding='same')(x)
        ...
        x = Dense(128, activation='relu', kernel_regularizer=l2(
            args.l2), kernel_initializer='he_uniform')(x)
        x = BatchNormalization()(x)
        # x = Dropout(0.5))
