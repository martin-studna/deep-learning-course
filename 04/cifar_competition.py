...
def main(args):
    ...
    train = train.shuffle(len(cifar.train.data["images"]), seed=args.seed).map(train_augment, num_parallel_calls=100, deterministic=False).prefetch(len(cifar.train.data["images"])//16).batch(args.batch_size)
