...
class VAE(tf.keras.Model):
    def __init__(self, args):
        ...
        for i in range(len(args.encoder_layers)):
            x = tf.keras.layers.Dense(args.encoder_layers[i], activation = 'relu')(x)
            
        z_mean = tf.keras.layers.Dense(args.z_dim)(x)
        z_log_variance = tf.keras.layers.Dense(args.z_dim)(x)

        self.encoder = tf.keras.Model(inputs=[input1], outputs=[z_mean, z_log_variance])
        ...
        input2 =  tf.keras.layers.Input(shape=[args.z_dim,])
        x = input2
        for i in range(len(args.decoder_layers)):
          x = tf.keras.layers.Dense(args.decoder_layers[i], activation = 'relu')(x)
        
        x = tf.keras.layers.Dense(MNIST.H * MNIST.W * MNIST.C, activation = 'sigmoid')(x)
        output = tf.keras.layers.Reshape((MNIST.H, MNIST.W, MNIST.C))(x)
        self.decoder = tf.keras.Model(inputs=[input2], outputs=[output])    
    ...
    def train_step(self, images):
        with tf.GradientTape() as tape:
            ...
            # standard deviation `exp(z_log_variance / 2)`. Sutart by creating
            # corresponding distribution `tfp.distribtions.Normal(...)` and then
            ...
            distr = tfp.distributions.Normal(z_mean, tf.math.exp(z_log_variance / 2) )
            ...
            reconstruction_loss = self.compiled_loss(images, decoded)
            ...
            latent_loss = tf.math.reduce_mean(distr.kl_divergence(self._z_prior)) 
            #latent_loss = distr.kl_divergence(self._z_prior)
            ...
            loss = tf.reduce_sum(  reconstruction_loss * images.shape[1] * images.shape[2] + latent_loss * self._z_dim ) 
