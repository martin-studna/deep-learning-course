...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        predictions = train.forms.word_mapping(words)
        ...
        output_layer = tf.keras.layers.Dense(
            train.tags.word_mapping.vocab_size())
        ...
        self._crf_weights = self.add_weight(shape=(train.tags.word_mapping.vocab_size(
        ), train.tags.word_mapping.vocab_size()), trainable=True, dtype=tf.float32, initializer=tf.keras.initializers.Zeros())
    ...
    def crf_loss(self, gold_labels, logits):
        assert isinstance(
            gold_labels, tf.RaggedTensor), "Gold labels given to CRF loss must be RaggedTensors"
        assert isinstance(
            logits, tf.RaggedTensor), "Logits given to CRF loss must be RaggedTensors"
        ...
        log_likelihood, transition_params = tfa.text.crf_log_likelihood(inputs=logits.to_tensor(), tag_indices=gold_labels.to_tensor(
    ...
    def crf_decode(self, logits):
        ...
        decode_tags, best_score = tfa.text.crf_decode(logits.to_tensor(
        ), transition_params=self._crf_weights, sequence_length=logits.row_lengths())
        predictions = tf.RaggedTensor.from_tensor(
            decode_tags, lengths=logits.row_lengths())
    ...
    class SpanLabelingF1Metric(tf.metrics.Metric):
        ...
        def update_state(self, y_true, y_pred, sample_weight=None):
            ...
            counts = tf.numpy_function(
                self.classify_spans, (y_true.values, y_pred.values, y_true.row_limits()), tf.int64)
...
def main(args):
    ...
    logs = network.fit(train, epochs=args.epochs,
                       validation_data=dev, callbacks=[network.tb_callback])
