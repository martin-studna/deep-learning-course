...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        # non_unique_words_idx = tf.where(
        #     unique_word_idx != tf.range(tf.size(flattened_words)))
        word_predictions = tf.gather(word_predictions, non_unique_words_idx)

        # TODO: Then, convert the flattened list into a RaggedTensor of the same shape
        # as `words` using `words.with_values` call.

        word_predictions = tf.RaggedTensor(words.with_values)(word_predictions)

        # TODO: Concatenate the word-level embeddings and the computed character-level WEs
        # (in this order).

        predictions = tf.concat(word_predictions, char_predictions)
