...
class Network(tf.keras.Model):
    def __init__(self, args, train):
        ...
        ones = tf.ones_like(word_predictions.shape, dtype=tf.float32)
        dropout_layer = tf.keras.layers.Dropout(rate=args.word_masking)
        dropout_results = dropout_layer.apply(ones)
        word_predictions = tf.where(dropout_results == 0, 0, word_predictions)
        ...
        flattened_word_predictions = tf.keras.layers.Flatten(
            data_format=words.values)(word_predictions)
        word_predictions, unique_word_idx = tf.unique(
            flattened_word_predictions)
        ...
        char_predictions = tf.strings.unicode_split(word_predictions, "UTF-8")
