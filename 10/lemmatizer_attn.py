...
class Network(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            ...
            loss = self.compiled_loss(
                y_targets.flat_values, y_pred.flat_values, regularization_losses=self.losses)
    ...
    def test_step(self, data):
        ...
        self.compiled_metrics.update_state(tf.ones_like(
            y.values, dtype=tf.int32), tf.cast(y_pred.values == y.values, tf.int32))
    ...
    class ShowIntermediateResults(tf.keras.callbacks.Callback):
        ...
        def on_train_batch_end(self, batch, logs=None):
            if network.optimizer.iterations % 10 == 0:
                ...
                tf.print(network.optimizer.iterations, forms[0, 0], lemmas[0, 0], network.predict_on_batch(
                    forms[:1, :1])[0, 0])
